...
int ezrtcp_sr_send(rtsp_con_t *rtspc) {
    dbg("\n");
    struct timeval tm;
    ...
    if (rtspc->faudioenb) {
        ...
        sr.ssrc = htonl(rtspc->session_audio.ssrc);
        sr.ntp_sec = htonl(ntp_sec);
        sr.ntp_frac = htonl(ntp_frac);
        sr.rtp_ts = htonl(rtspc->session_audio.ts);
        sr.psent = 0;
        sr.osent = 0;
        ...
        if (rtspc->fovertcp) {
            buf[0] = '$';
            buf[1] = (rtspc->session_audio.trackid*2)+1;
        ...
        fd = (rtspc->fovertcp) ? rtspc->evt->fd : rtspc->session_audio.fd_rtcp;
    ...
    if (rtspc->fvideoenb) {
        ...
        sr.ssrc = htonl(rtspc->session_video.ssrc);
        sr.ntp_sec = htonl(ntp_sec);
        sr.ntp_frac = htonl(ntp_frac);
        sr.rtp_ts = htonl(rtspc->session_video.ts);
        sr.psent = 0;
        sr.osent = 0;
        ...
        if (rtspc->fovertcp) {
            buf[0] = '$';
            buf[1] = (rtspc->session_video.trackid*2)+1;
            PUT_16(buf + 2, sizeof(sr));
            bufn += 4;
        }
        memcpy(buf + bufn, &sr, sizeof(sr));
        ...
        fd = (rtspc->fovertcp) ? rtspc->evt->fd : rtspc->session_video.fd_rtcp;
        if (0 != ezrtp_packet_send(fd, (void*)buf, bufn)) {
